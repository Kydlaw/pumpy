...
class TwitterAuth(object):
    def __init__(
        self, mode, access_token, access_token_secret, api_key=None, api_key_secret=None
    ):
        if mode != "stream" or mode != "get":
            print("The 'mode' argument should be 'stream' or 'get'")
        if api_key is not None and api_key_secret is not None:
            self.api_key = api_key
            self.api_key_secret = api_key_secret
        self.access_token = access_token
        self.access_token_secret = access_token_secret

        auth = AppAuthHandler(access_token, access_token_secret)
...
class OAuthHandler(AuthHandler):
    ...
    def __init__(self, consumer_key, consumer_secret, callback=None):
        ...
        self.oauth = OAuth1Session(
            consumer_key, client_secret=consumer_secret, callback_uri=self.callback
        )
